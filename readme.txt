...
实验目的:
	学习ATK-AS608指纹识别模块的通讯协议最终完成录指纹、删除指纹、刷指纹等功能.
	
硬件资源:	
	1,蜂鸣器（PF8）
	2,GPIO（PA6）读取指纹模块触摸感应信号
	3,按键KEY0(PE4)/KEY1(PE3)/KEY2(PE2)/KEY_UP(PA0,也称之为WK_UP)
	4,串口1(波特率:115200,PA9/PA10连接在板载USB转串口芯片CH340上面)用于支持USMART调试
	5,串口2(波特率:57600,PA2/PA3)与指纹模块AS608通讯
	6,ALIENTEK 2.8/3.5/4.3/7寸TFTLCD模块(通过FSMC驱动,FSMC_NE4接LCD片选/A6接RS) 	
	7,W25Q128(SPI FLASH芯片,连接在SPI1上，用于存储字库) 
	8,24C02（用于存储LCD触摸屏一些参数）
	9,ATK-AS608指纹识别模块1个,杜邦线6根
...
注意事项:
	...
	4,使用2.8寸或3.5寸屏（电阻屏）初始化提示“是否进行触摸屏校准”？一般选择“否”实验过程发现触摸不准则复位再进行校准！
	5,本例程的详细说明，见光盘：9，增值资料\1，ALIENTEK 产品资料\4，ATK-AS608 指纹识别模块\ATK-AS608模块使用说明(探索者F4版)_AN1601C.pdf
