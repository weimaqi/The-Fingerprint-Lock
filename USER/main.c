...
#define jpeg_buf_size 31*64  			//¶¨ÒåJPEGÊý¾Ý»º´æjpeg_bufµÄ´óÐ¡(*4×Ö½Ú)
__align(4) u32 jpeg_buf[jpeg_buf_size];	//JPEGÊý¾Ý»º´æbuf
volatile u32 jpeg_data_len=0; 			//bufÖÐµÄJPEGÓÐÐ§Êý¾Ý³¤¶È 
...
const u8*EFFECTS_TBL[7]={"Normal","Negative","B&W","Redish","Greenish","Bluish","Antique"};	//7ÖÖÌØÐ§ 
...
void jpeg_data_process(void)
{
	if(ov2640_mode)//Ö»ÓÐÔÚJPEG¸ñÊ½ÏÂ,²ÅÐèÒª×ö´¦Àí.
	{
		...
		{	
			...
			jpeg_data_len=jpeg_buf_size-DMA_GetCurrDataCounter(DMA2_Stream1);//µÃµ½´Ë´ÎÊý¾Ý´«ÊäµÄ³¤¶È
...
void jpeg_test(void)
{
  ...
  POINT_COLOR=RED; 
	...
	LCD_ShowString(30,140,200,16,16,"KEY2:Effects"); 			//ÌØÐ§ 
	...
	while(1)
	{
		...
		{  
			Mark = 0;
			p=(u8*)jpeg_buf;
			LCD_ShowString(30,210,210,16,16,"Sending JPEG data..."); //ÌáÊ¾ÕýÔÚ´«ÊäÊý¾Ý
			...
			if(key)	//ÓÐ°´¼ü°´ÏÂ,ÐèÒª´¦Àí
			{  
				LCD_ShowString(30,210,210,16,16,"Quit Sending data   ");//ÌáÊ¾ÍË³öÊý¾Ý´«Êä
				switch(key)
				{				    
					...
					case KEY1_PRES:	//±¥ºÍ¶ÈSaturation
						...
						break;
					case KEY2_PRES:	//ÌØÐ§ÉèÖÃ				 
						effect++;
						if(effect>6)effect=0;
						OV2640_Special_Effects(effect);//ÉèÖÃÌØÐ§
				...
				LCD_ShowString(30,180,210,16,16,msgbuf);//ÏÔÊ¾ÌáÊ¾ÄÚÈÝ
				delay_ms(800); 				  
...
{ 
	...
	LCD_ShowString(30,150,200,16,16,"KEY2:Effects"); 			//ÌØÐ§ 
...
int KeyBoardScan(){
	...
	if(GPIO_ReadInputDataBit(GPIOF,GPIO_Pin_4) | GPIO_ReadInputDataBit(GPIOF,GPIO_Pin_5) | GPIO_ReadInputDataBit(GPIOF,GPIO_Pin_6) | GPIO_ReadInputDataBit(GPIOF,GPIO_Pin_7)){
			delay_ms(100);
		if(GPIO_ReadInputDataBit(GPIOF,GPIO_Pin_4) | GPIO_ReadInputDataBit(GPIOF,GPIO_Pin_5) | GPIO_ReadInputDataBit(GPIOF,GPIO_Pin_6) | GPIO_ReadInputDataBit(GPIOF,GPIO_Pin_7)){
			GPIOF->BSRRL= GPIO_Pin_0;       //ÅÐÊÇ²»ÊÇµÚÒ»ÐÐ
			...
			GPIOF->BSRRL= GPIO_Pin_1;       //ÅÐÊÇ²»ÊÇµÚ¶þÐÐ
			...
			GPIOF->BSRRL= GPIO_Pin_2;       //ÅÐÊÇ²»ÊÇµÚÈýÐÐ
			...
			GPIOF->BSRRL= GPIO_Pin_3;       //ÅÐÊÇ²»ÊÇµÚËÄÐÐ
...
//	num=GET_NUM();//»ñÈ¡·µ»ØµÄÊýÖµ
	...
	{
		LCD_Fill(0,120,lcddev.width,160,WHITE);
		LCD_ShowString(30,110,200,16,16,"É¾³ýÖ¸ÎÆ³É¹¦");
	}
  else
		ShowErrMessage(ensure);	
	delay_ms(1200);
	ensure=PS_ValidTempleteNum(&ValidN);//¶Á¿âÖ¸ÎÆ¸öÊý È·ÈÏ×Ö
                                      //Êä³öÈ·ÈÏÇé¿ö
...
//				LCD_Fill(0,100,lcddev.width,160,WHITE);
			...
			case 4:	
				...
				while(!(ID<AS608Para.PS_max));//ÊäÈëID±ØÐëÐ¡ÓÚÄ£¿éÈÝÁ¿×î´óµÄÊýÖµ
				...
				{			
					...
					LCD_ShowNum(56,80,AS608Para.PS_max-ValidN,3,16);//ÏÔÊ¾Ê£ÓàÖ¸ÎÆÈÝÁ¿
					delay_ms(1500);
...
//	Esp8266Init();
	...
	ov2640_mode=1;
	TIM6_Int_Init(10000,7199);			//10Khz¼ÆÊýÆµÂÊ,1ÃëÖÓÖÐ¶Ï	
...
/*****************************/
	...
	for(j=0;j<6;j++){
		...
		USART_SendData(USART3,test[j]); 	 //·¢ËÍÊý¾Ýµ½´®¿Ú3 
	}
